-- Initialise automatiquement chaque nouvelle semaine ? 01:00 (heure de Paris).
-- Les administrateurs peuvent toujours fermer individuellement un jour ensuite.

ALTER TABLE public.weekly_room_days
  ALTER COLUMN is_open SET DEFAULT TRUE;

UPDATE public.weekly_room_days
SET is_open = TRUE, updated_at = NOW()
WHERE attendance_date BETWEEN
  date_trunc('week', timezone('Europe/Paris', NOW()) - INTERVAL '1 hour')::DATE
  AND (date_trunc('week', timezone('Europe/Paris', NOW()) - INTERVAL '1 hour')::DATE + 6)
  AND NOT EXISTS (
    SELECT 1
    FROM public.weekly_room_attendance attendance
    WHERE attendance.attendance_date = weekly_room_days.attendance_date
  );
...
BEGIN
  IF p_attendance_date NOT BETWEEN current_week_start AND (current_week_start + 6) THEN
