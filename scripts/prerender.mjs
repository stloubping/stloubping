...
const shell = (html, page) => {
  const content = `<main data-prerendered="true"><h1>${escapeHtml(page.heading)}</h1><p>${escapeHtml(page.text)}</p>${publicContent[page.path] ?? ""}<p>Saint-Loub'Ping â€” tennis de table à Saint-Loubès, Gironde (33).</p></main>`;
