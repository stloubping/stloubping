...
const StageAout = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!Object.values(selected).some(Boolean)) return toast.error("Choisissez au moins une journee.");
    setSaving(true);
    const { error } = await supabase.from("august_stage_registrations").insert({ first_name: firstName.trim(), last_name: lastName.trim(), ...selected });
    if (error) toast.error("L'inscription n'a pas pu etre enregistree.");
    else { setFirstName(""); setLastName(""); setSelected(emptyDays()); await refresh(); toast.success("Inscription enregistree."); }
    setSaving(false);
  };
  const print = () => {
