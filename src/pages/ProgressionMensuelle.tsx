...
const ProgressionMensuelle = () => {
  ...
  // Tri par progression mensuelle décroissante
  const sortedPlayers = useMemo(() => {
    return [...players]
      ...
      .sort((a, b) => (b.progmens || 0) - (a.progmens || 0));
  }, [players, searchQuery]);

  // Statistiques clés
  const topMonthlyPlayer = sortedPlayers.length > 0 ? sortedPlayers[0] : null;
  const positiveGainsCount = players.filter(p => (p.progmens || 0) > 0).length;
  const maxGain = topMonthlyPlayer?.progmens || 0;
    ...
    <div className="container mx-auto px-4 py-8 bg-clubLight text-clubLight-foreground">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl md:text-5xl font-extrabold text-clubDark mb-3 flex items-center justify-center gap-3">
          <Calendar className="h-8 w-8 md:h-10 md:w-10 text-clubPrimary" />
          Progression Mensuelle
        </h1>
        <p className="text-muted-foreground text-sm md:text-base max-w-2xl mx-auto">
          Découvrez l'évolution des points de nos licenciés sur le dernier mois enregistrée par la FFTT et Pingpocket.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <Card className="bg-clubDark text-white shadow-xl border-none rounded-xl">
          <CardHeader className="p-4 pb-2">
            <CardDescription className="text-gray-300 text-xs flex items-center gap-1.5 font-medium">
              <Zap className="h-4 w-4 text-yellow-400" /> N°1 du Mois
            </CardDescription>
            <CardTitle className="text-lg md:text-xl font-bold truncate text-white">
              {topMonthlyPlayer ? (
                <span>{topMonthlyPlayer.nom} {topMonthlyPlayer.prenom} <span className="text-emerald-400 font-extrabold">(+{maxGain} pts)</span></span>
              ) : (
                "Chargement..."
              )}
            </CardTitle>
        ...
        <Card className="bg-clubDark text-white shadow-xl border-none rounded-xl">
          <CardHeader className="p-4 pb-2">
            <CardDescription className="text-gray-300 text-xs flex items-center gap-1.5 font-medium">
              <TrendingUp className="h-4 w-4 text-emerald-400" /> Joueurs en hausses
            </CardDescription>
            ...
            <div>
              <CardTitle className="text-xl md:text-2xl font-bold text-clubDark flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-[10px] border-clubPrimary text-clubPrimary bg-clubPrimary/10">
                  <Award className="h-3 w-3 mr-1" /> Mis à jour
                </Badge>
              </CardTitle>
              <CardDescription className="text-xs md:text-sm text-muted-foreground">
                Trié automatiquement de la plus forte progression à la baisse du mois.
              </CardDescription>
            </div>
        ...
        <CardContent className="p-0 md:p-6 pt-2">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm font-semibold text-clubDark">Chargement des données de progression...</p>
            </div>
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow className="bg-clubDark hover:bg-clubDark">
                    ...
                    <TableHead className="text-white font-bold text-center text-xs">Gain du Mois</TableHead>
                ...
                <TableBody>
                  {sortedPlayers.map((player, index) => {
                    const gainVal = player.progmens || 0;
