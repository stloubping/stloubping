...
type WackOrder = { id: string; order_number: string; ordered_at: string; received_at: string | null; paid_at: string | null; amount: number };
type Draft = Omit<WackOrder, "id">;
const emptyDraft = (): Draft => ({ order_number: "", ordered_at: new Date().toISOString().slice(0, 10), received_at: null, paid_at: null, amount: 0 });
...
const WackSportOrders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("wacksport_orders").select("id, order_number, ordered_at, received_at, paid_at, amount").order("ordered_at", { ascending: false });
  ...
  const edit = (order: WackOrder) => { setEditId(order.id); setDraft({ order_number: order.order_number, ordered_at: order.ordered_at, received_at: order.received_at, paid_at: order.paid_at, amount: Number(order.amount) }); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    const payload = { order_number: draft.order_number.trim(), ordered_at: draft.ordered_at, received_at: draft.received_at, paid_at: draft.paid_at, amount: draft.amount };
  ...
  return <div className="min-h-screen bg-clubLight">
    ...
    <main className="container mx-auto space-y-7 px-4 py-8">
      <Card><CardHeader><CardTitle>{editId ? "Modifier la commande Wack Sport" : "Ajouter une commande Wack Sport"}</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="flex gap-2 md:col-span-2 xl:col-span-5 xl:justify-end">{editId && <Button type="button" variant="outline" onClick={reset}><X className="mr-2 h-4 w-4" /> Annuler</Button>}<Button type="submit" disabled={saving} className="bg-clubPrimary font-bold hover:bg-clubPrimary/90">{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : editId ? <Save className="mr-2 h-4 w-4" /> : <Plus className="mr-2 h-4 w-4" />}{editId ? "Enregistrer" : "Ajouter la commande"}</Button></div>
      </form></CardContent></Card>
      <Card className="overflow-hidden"><div className="hidden grid-cols-[1.2fr_1fr_repeat(3,1fr)_auto] gap-4 bg-clubDark px-5 py-4 text-sm font-bold text-white lg:grid"><span>N° de commande</span><span>Somme</span><span>Commandée le</span><span>Réceptionnée le</span><span>Payée le</span><span>Action</span></div><div className="divide-y">{loading ? <div className="flex justify-center py-14"><Loader2 className="h-8 w-8 animate-spin text-clubPrimary" /></div> : orders.length === 0 ? <p className="p-12 text-center text-muted-foreground">Aucune commande Wack Sport enregistrée.</p> : orders.map((order) => <div key={order.id} className="grid gap-3 p-5 lg:grid-cols-[1.2fr_1fr_repeat(3,1fr)_auto] lg:items-center"><Value label="N°" value={order.order_number} strong /><Value label="Somme" value={money(Number(order.amount))} strong /><Value label="Commandée le" value={prettyDate(order.ordered_at)} /><Value label="Réceptionnée le" value={prettyDate(order.received_at)} /><Value label="Payée le" value={prettyDate(order.paid_at)} /><Button variant="outline" size="sm" onClick={() => edit(order)}><Pencil className="mr-2 h-4 w-4" /> Modifier</Button></div>)}</div></Card>
    </main>
  </div>;
};
