...
const EquipmentOrdersRecap = () => {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("equipment_orders").select("id, created_at, first_name, last_name, items, status, paid_at, handed_over_at, handed_over_by").in("status", statuses).order("created_at", { ascending: false });
  ...
  const save = async (order: Order) => {
    ...
    else { setOrders((current) => current.map((entry) => entry.id === order.id ? { ...entry, first_name: draft.first_name.trim(), last_name: draft.last_name.trim(), items, paid_at: draft.paid_at || null, handed_over_at: draft.handed_over_at || null, handed_over_by: draft.handed_over_by.trim() || null } : entry)); setEditingId(null); setDraft(null); toast.success("Récapitulatif mis à jour."); }
    setSaving(false);
  };
  ...
  return <div className="min-h-screen bg-clubLight">
    ...
    <main className="container mx-auto px-4 py-8">{loading ? <div className="flex justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-clubPrimary" /></div> : orders.length === 0 ? <Card><CardContent className="py-14 text-center text-muted-foreground">Aucune commande transmise au fournisseur.</CardContent></Card> : <div className="space-y-4">
      ...
      <Card className="overflow-hidden"><div className="overflow-x-auto">
        <div className="hidden min-w-[1380px] grid-cols-[.8fr_.8fr_.65fr_.8fr_.8fr_.8fr_1fr_1.7fr] gap-4 bg-clubDark px-5 py-4 text-sm font-bold text-white md:grid"><span>Nom</span><span>Prénom</span><span>Somme</span><span>N° de commande</span><span>Payé le</span><span>Donné le</span><span>Donné par</span><span>Action</span></div>
        <div className="divide-y">{orders.map((order) => { const editingDraft = editingId === order.id ? draft : null; return <div key={order.id} className="grid gap-4 p-5 md:min-w-[1380px] md:grid-cols-[.8fr_.8fr_.65fr_.8fr_.8fr_.8fr_1fr_1.7fr] md:items-center">{editingDraft ? <>
          <Input aria-label="Nom" value={editingDraft.last_name} onChange={(e) => setDraft({ ...editingDraft, last_name: e.target.value })} />
          <Input aria-label="Prénom" value={editingDraft.first_name} onChange={(e) => setDraft({ ...editingDraft, first_name: e.target.value })} />
          <Input aria-label="Somme" type="number" min={0} step="0.01" value={editingDraft.total} onChange={(e) => setDraft({ ...editingDraft, total: e.target.value })} />
          <Input aria-label="Numéro de commande" value={editingDraft.order_number} onChange={(e) => setDraft({ ...editingDraft, order_number: e.target.value })} />
