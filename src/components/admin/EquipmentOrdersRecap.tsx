...
const EquipmentOrdersRecap = () => {
  ...
  return <div className="min-h-screen bg-clubLight">
    ...
    <main className="container mx-auto px-4 py-8">{loading ? <div className="flex justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-clubPrimary" /></div> : orders.length === 0 ? <Card><CardContent className="py-14 text-center text-muted-foreground">Aucune commande transmise au fournisseur.</CardContent></Card> : <Card className="overflow-hidden">
      <div className="hidden grid-cols-[18%_18%_14%_18%_32%] gap-4 bg-clubDark px-5 py-4 text-sm font-bold text-white md:grid"><span>Nom</span><span>Prénom</span><span>Somme</span><span>N° de commande</span><span>Action</span></div>
      <div className="divide-y">{orders.map((order) => { const editingDraft = editingId === order.id ? draft : null; return <div key={order.id} className="grid gap-4 p-5 md:grid-cols-[18%_18%_14%_18%_32%] md:items-center">{editingDraft ? <>
